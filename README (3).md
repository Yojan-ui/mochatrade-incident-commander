...
**A crisis cockpit for a flash crash: it detects a liquidation cascade, escalates it automatically, lets one person stop it market by market (even without touching the mouse), drafts the public message, writes a tamper-evident record of every decision and exports a sign-off-ready PDF post-mortem.**
...
Each tab is also a status light: an asset tab shows that market's live **stress**, e.g. `1.4×`, with a green/yellow/red lamp, or `BRK` (cyan) when a circuit breaker is on. Keys `1`–`4` or the arrow keys switch tabs.

### Hotkeys ("zero-mouse" workflow)
A muted legend at the very bottom of the screen lists them.

| Key | Action |
|---|---|
| `SPACE` or `⌘K` / `Ctrl+K` | **Pause all cascading markets** at once. Markets already paused or halted are left alone, and the legend confirms e.g. `→ paused 4 markets` |
| `1` `2` `3` `4` | Switch to OVERVIEW, BTC, ETH, SOL |
| `ENTER` | Press the focused button |

- **Typing is safe:** hotkeys never fire while you're typing in a field. ⌘K works everywhere.
- **Space is always the panic key:** it pauses everything even when a button has focus, so a focused "Resume" button can never turn the panic key into a resume.
- **Logged:** each global pause is written to the incident log as one entry, e.g. "Global pause via [SPACE]: …".
...
6. **Incident log:** a tamper-evident decision record.
   - Every action, escalation, ruling, SLA change and comms step is added automatically, and team members can append their own notes.
   - `CHAIN OK` means the hash chain verifies.
   - **EXPORT .log** downloads the raw ledger.
...
|---|---|---|---|
| 1–20 | `PRE-CRASH` | ~0.06–0.12 | ~$1M/min |
| 21–40 | `FLASH CRASH` | exponential: `0.1 × e^(0.113·(t−20))`, about 0.11 → 0.31 → 0.54 → 0.95 at ticks 21, 30, 35, 40 | $1.3M → $57M/min |
| 41–100 | `SUSTAINED` | 0.78–1.00 (average 0.93), random swings | ~$43–63M/min |
...
**Shockwaves.** When the market's live liquidations jump by more than 2% in one tick, a ring expands from mid at 0.2 surface-widths per second. It adds a tremor of extra loss (`+0.3·ring·nearMid + 0.12·ring`), so it never does the cave-in's job:
```
...
### 3.8 Running totals (for the post-mortem)
Rates are per minute, so each tick adds `rate × minutes elapsed` since the last tick:
```
total liquidation volume ← total + volume_rate × Δt_min
liquidations executed    ← total + liquidation_rate × Δt_min
peak rate                ← max(peak, volume_rate)
```
Totals start when the dashboard begins monitoring, and the report states that time.

### 3.9 PDF post-mortem
**GENERATE POST-MORTEM** loads **jsPDF** on first click and builds an A4 report in Courier.
1. **Summary metrics:**
   - **Total liquidation volume**, the **Fault-vs-Market ruling** (or `NOT RECORDED`) and **time to resolution**;
   - then time to first mitigation, peak severity, liquidations, peak rate, tickets, broadcasts, SLA, US-market state and incident open time (UTC and IST).
2. **Markets at export:** status, mark, 24h change, volume.
3. **Follow-ups:** generated from state, e.g. "run the compensation job", "announce reopening for ETH-PERP (HALTED)".
4. **Decision record:** every ledger entry with its hashes, and the chain marked `VERIFIED`. Entries never split across pages.
5. **Sign-off lines:** Incident Commander, Head of Risk, Compliance Officer.

Every page has a `CONFIDENTIAL // INC-2417 // ledger head … // PAGE n OF N` footer.

- **Time to resolution** = incident opened → post-mortem export. It's marked `UNRESOLVED at export` if SEV-1 is still active.
- **Time to first mitigation** = incident opened → the first pause or halt.
- **Symbols:** Courier can't print `≥ → × …`, so the PDF spells them `>= -> x ...`.

### 3.10 Other precise details
- **SLA timer:** `remaining = ⌈(deadline − now)/1000⌉`. It counts against a fixed deadline, so throttled browser timers can't make it drift. Type new minutes and press Enter to restart it; the change is logged.
...
| 3D | three.js + react-three-fiber + custom GLSL shader (drei for the 3D text). Loaded on demand |
| PDF | jsPDF, loaded on first "GENERATE POST-MORTEM" click. The main bundle stays ~452 KB |
...
| 2:35 | Scroll to **Incident log**, click **GENERATE POST-MORTEM** | "Every click is here, hash-chained. CHAIN OK. One click and it's a sign-off-ready PDF for compliance: total volume, ruling, time to resolution, the full decision record." Open the PDF. |
| 2:55 | Close | "Detect, escalate, act, communicate, record. One screen, one person, under pressure." |
...
- The PDF post-mortem already exists; production would sign the PDF (and the ledger head it quotes) server-side, so a regulator can verify it wasn't altered after export.
- The India angle matters here: clear, timestamped records of halts, compensation and customer comms.
...
- **Provable.** A tamper-evident log of every action with UTC/IST time, exported in one click as a sign-off-ready PDF post-mortem.
- **Built for pressure.** One hotkey pauses every cascading market; number keys jump between assets. No hunting for buttons mid-crisis.
...
It's sign-off-ready and tamper-evident: it quotes the ledger's head hash, so its decision record can be re-verified. It isn't cryptographically signed in the demo; production would sign it server-side.

**What if someone presses Space by accident?**
It only pauses liquidations (the least harmful brake, not a halt), it's logged, and every market can be resumed individually.
